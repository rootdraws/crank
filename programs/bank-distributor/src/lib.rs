// bank_distributor — cumulative Merkle-based BANK distribution
//
// Identical mechanics to merkle-distributor (SOL); deployed under a different
// program ID so the daily SOL distributor and BANK distributor coexist as two
// independent singletons. Same `[b"distributor"]` PDA seed scoped per-program.
//
// Each epoch the bot:
//   1. Computes per-wallet trader weights (same `computeShares` as the SOL tree).
//   2. Builds a Merkle tree of (wallet, cumulative_bank_amount) leaves.
//   3. Pins the tree JSON to IPFS.
//   4. Calls new_epoch to upload root + IPFS CID + fund the vault with BANK
//      from the rover_burn_and_mint output.
//
// Claims are cumulative — each root represents total lifetime BANK entitlements.
...
declare_id!("9sqcwp65VGxkbLG3KN85BrzZz2Q77xnfbpPcBfn1kj7M");
...
pub mod bank_distributor {
    ...
    /// the vault is pre-funded by program flows (rover_burn_and_mint for BANK,
    /// drain_vault + sync_native for SOL), so this instruction never moves
    /// tokens — it only records the root and computes this epoch's amount
    /// as the delta that appeared in the vault since last epoch.
    pub fn new_epoch(
        ...
        // epoch_amount = (vault_balance + total_amount_claimed) - total_amount_funded.
        // Conceptually: whatever new tokens showed up in the vault since we last
        // published a root, after accounting for what's been paid out in claims.
