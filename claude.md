...
**Treasury (Path B):**
- **Marker (governance-signed):** `authorize_treasury_open`, `authorize_treasury_close` mint a `TradeAuth` PDA bound to a specific user vault + side + amount. Whitelisted; only passable via the addin. `wrap_caller_sol(amount)` is a generic governance-callable utility prepended to SOL-buy markers — wraps caller's native lamports → caller-owned WSOL ATA via `SystemProgram::transfer + sync_native`. Constrained by `token::authority = caller` so an NTP-signed call can only target NTP's WSOL ATA.
...
- **Bot recovery:** `cancel_expired_trade_auth` closes a stuck TradeAuth past its TTL when `expires_at < now`. Required because `init` on the marker ix blocks any new TradeAuth while a stale one exists for the same user_vault — and there's no other path to close one outside of consume. Rent → bot. Diagnostic + invocation: `scripts/check-trade-auth.ts`.
...
scripts/
  ...
  close-all-positions.ts, drain-residue.ts, drain-treasury-native.ts
...
- **Price source per token:** SOL routes to Pyth Hermes (via `fetchDexScreenerPrice(SOL_MINT)` shim). CRANK reads PumpSwap on-chain reserves directly via `fetchPumpSwapPriceUsd` — `curator.json` sets `priceSource: "pumpswap"` + `pumpswapPool`. Other tokens fall back to DexScreener with stablecoin-pair preference.
...
- **Path B per-fire SOL cost (~0.0005–0.001 SOL).** Bot pays tx fees on every ix in the marker → execute → close cycle, plus per-ix gas (no longer reimbursed from user_vault post-2026-05-10 rework). Bin-array rent + Position/Vault/PositionSettle/TradeAuth PDA rent all round-trip the bot net-zero. Not material at current volume.
- **Anchor 0.30 optional accounts use compact form.** `Option<Account<...>>` accounts that are None are OMITTED from the keys array entirely — Anchor does NOT pad with the program ID placeholder (older convention). Symptoms when this is wrong: on-chain `AnchorError caused by account: system_program. InvalidProgramId. Left: <bin-farm program> Right: 11111…`. Diagnostic: `solana confirm <sig> -v` and count keys vs IDL. Anchor TS `.accounts({...})` is broken for ixs with optionals (passing `null` shifts indices; omitting throws "Account X not provided"). Hand-craft the keys array directly. See `packages/discord-bot/src/commands/close.ts` (user_close) and `bot/harvest-executor.ts` (close_position) for the pattern. Latent bug warnings live in `scripts/{force-close-position,close-all-positions,close-treasury-positions}.ts` — those still use the broken Anchor builder; mirror the hand-crafted form before next use.
- **Path B `bin_array_bitmap_ext` writability must match between open and close ixs.** `treasury_open_combined` passes it writable; `treasury_close_combined` did not (early version) → "writable privilege escalated" during Meteora CPI. `buildTreasuryCloseCombinedIx` now flips writable when bitmap-ext is not the DLMM-program sentinel. Same flip applies to any hand-rolled close ix that calls Meteora's `remove_liquidity` family.
- **NTP WSOL ATA delegate is required for /buy SOL Path B.** `treasury_open_combined` reads `caller_token_account` from NTP's WSOL ATA but bot signs the deposit. Bot needs delegate authority via legacy SPL Token `ApproveChecked` (NOT Token-2022 — WSOL is legacy). One-shot governance proposal: `scripts/bot-delegate-setup-wsol.ts`. Diagnostic + cancel-expired-marker: `scripts/check-trade-auth.ts`. The legacy SPL Token `ApproveChecked` (variant 0x0D, 1-byte disc) must be on the addin whitelist.
...
- **Path B drain protection:** even with FFwq compromised, a Path B drain proposal cannot pass — the proposal-whitelist-addin returns voter weight 0 for any community-side proposal whose inner ixs aren't all on the registrar whitelist. Canonical whitelist in `scripts/update-addin-whitelist.ts`. The marker ixs bind a TradeAuth to a specific user vault + side + amount; the bot's direct `treasury_*_combined` tx that consumes the TradeAuth has on-chain constraints (`ntp_crank_ata.owner == user_vault.owner`, `ntp_sol_account == user_vault.owner`) preventing routing to attacker-controlled accounts. `wrap_caller_sol` is constrained by `token::authority = caller` so an NTP-signed call can only target NTP's own WSOL ATA. Council axis is ungated — emergency / admin operations require the council mint, currently held only by the operator's hot wallet. Once HW rotates, council mint moves to HW.
...
- **Treasury holdings (NTP-direct, Realms-visible):** 1M CRANK in NTP's CRANK ATA + 1.472 SOL native on NTP. Orphan inventory from pre-marker testing (1M CRANK in `treasury_user_vault`'s ATA, invisible to Realms) drained back to NTP via one-shot `withdraw_treasury_token` proposal. 0.993 SOL of pre-rework operational float drained user_vault → NTP via `drain_treasury_native_to_ntp` (2026-05-10).
...
- **Path B SOL-buy unblock (2026-05-10):** added `wrap_caller_sol` ix (NTP native lamports → NTP WSOL ATA, atomically prepended to SOL-buy markers) + `cancel_expired_trade_auth` (bot recovery for stuck markers) + bot delegate setup over NTP's WSOL ATA via legacy SPL Token `ApproveChecked` (~1M SOL approved). Bot match-size source switched from treasury vault native lamports → NTP native lamports. CRANK price source switched DexScreener → PumpSwap on-chain reserves. Anchor 0.30 optional-account compact-form fix applied to `discord-bot/src/commands/close.ts` (user_close) and `bot/harvest-executor.ts` (close_position) — previous `.accounts()` calls threw `InvalidProgramId` on system_program due to None-optionals being padded instead of omitted.
