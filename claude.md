...
- **Discord bot live** — `crankbot#8555`, 13 slash commands, feed channel `#crank-feed`. `/burn` now supports `/burn status` sub-mode (reads curve state). `/start` auto-creates vault's CRANK ATA (Token-2022). Minimum vault SOL UX copy: 0.05 SOL (was 0.25).
