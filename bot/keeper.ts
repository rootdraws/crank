 ...
 * keeper.ts
 *
 * Daily orchestration for crank.money. Post-2026-04-28 pivot: BANK distribution,
 * gauge-weighted Merkle epochs, and rover burn-and-mint are retired. The
 ...
 *   2. sweep_rover              — native SOL split via on-chain curve
 *   3. open_fee_rovers          — token ATAs → DLMM positions
 *   4. close_exhausted_rovers   — empty rovers → rent reclaimed
 *   5. prune_inactive_members   — drop crank role from idle Discord members
 *   6. refresh_supplies         — refresh in-memory CRANK supply for /buy math
 *   7. daily_stats_post         — post protocol-wide volume summary to #crank-stats
 *
...
export class MonkeKeeper {
  ...
  async runDailySequence(): Promise<string> {
    ...
    // Step 2: Sweep SOL from rover_authority via the on-chain curve.
    //         (Curve-sweep machinery is slated for retirement in the
    //         follow-up bin-farm cleanup upgrade — left in place for now.)
    await this.crankSweepRover();

    // Step 3: Open fee rover positions from accumulated token fees.
    await this.crankOpenFeeRovers();

    // Step 4: Close exhausted rover positions (reclaim rent).
    await this.crankCloseExhaustedRovers();

    // Step 5: Prune the crank role from idle Discord members (best-effort).
    await this.crankPruneInactiveMembers();

    // Step 6: Refresh in-memory pool supplies from on-chain mint state.
    await this.crankRefreshSupplies();

    // Step 7: Post a daily volume summary to #crank-stats.
  ...
  private async crankOpenFeeRovers(): Promise<void> {
    try {
      ...
      // CRANK fee accumulation is left in place pending the bin-farm cleanup upgrade.
