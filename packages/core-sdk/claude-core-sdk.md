...
| File | What it does |
|------|-------------|
| `constants.ts` | All program IDs (5 programs + Meteora DLMM), token mints (CRANK, BANK, PEGGED, USDC), LbPair byte layout offsets, protocol constants. SOURCE OF TRUTH. |
...
**Claiming $PEGGED (/claim):**
`getOrCreate` → `getDistributorPDA` → read IPFS CID from chain → fetch Merkle tree → `getClaimStatusPDA` → raw `claim` ix → `signAndSend`
